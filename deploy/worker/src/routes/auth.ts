...
authRouter.post('/register', async (c) => {
  try {
    ...
    try {
      ...
      return c.json({ success: false, error: 'DB query failed' }, 500);
    }
    if (existing) {
    ...
    try {
      ...
      return c.json({ success: false, error: 'DB insert failed' }, 500);
    }
    ...
    return c.json({ success: false, error: 'Registration failed', detail: String(err) }, 500);
  }
});
